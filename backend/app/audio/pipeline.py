...
class IntelligencePipeline:
    ...
    def analyze_bytes(self, data: bytes, context: SensorContext, filename: str = "capture.wav") -> dict[str, Any]:
        y, sr = load_audio(data, sr=settings.sample_rate, filename=filename)
    ...
    def analyze_array(
        ...
        # Ensure minimum length for DSP processing
        if len(y) < sr // 4:
            y = np.pad(y, (0, max(0, sr // 4 - len(y))))

        dsp_hits = specialist_detect(y, sr, filename=filename)
        events = fuse_events(yamnet_hits, panns_hits, dsp_hits)

        # Compute physical DSP acoustic features safely
        rms = float(np.sqrt(np.mean(y * y) + 1e-12))
        zcr = float(np.mean(np.abs(np.diff(np.sign(y)))) / 2.0) if len(y) > 1 else 0.0
        nperseg = max(64, min(2048, len(y)))
        from scipy import signal as scipy_signal
        try:
            freqs, psd = scipy_signal.welch(y, fs=sr, nperseg=nperseg)
            psd = psd + 1e-18
            total_p = np.sum(psd)
            dominant_hz = float(freqs[np.argmax(psd)]) if len(psd) > 0 else 340.0
            centroid = float(np.sum(freqs * psd) / (total_p + 1e-12)) if total_p > 0 else 1500.0
        except Exception:
            dominant_hz = 340.0
            centroid = 1500.0

        acoustic_features = {
            "dominant_frequency": round(dominant_hz, 1),
        ...
        top_event_obj = events[0] if events else {}
        event_label = top_event_obj.get("label", "Detected Audio Event")
        category = top_event_obj.get("category", "acoustic")
        confidence = float(top_event_obj.get("score", 0.88))

        cat_low = category.lower()
        lbl_low = event_label.lower()
        if any(k in cat_low or k in lbl_low for k in ["gun", "blast", "rifle", "weapon", "explosion", "shot"]):
            threat_level = "CRITICAL"
        elif any(k in cat_low or k in lbl_low for k in ["drone", "uav", "aircraft", "helicopter", "rotor", "aerial", "scream", "distress"]):
            threat_level = "HIGH"
        elif any(k in cat_low or k in lbl_low for k in ["crowd", "clap", "alarm", "siren", "shatter", "glass"]):
            threat_level = "MEDIUM"
        else:
            threat_level = "LOW"
        ...
        payload = {
            ...
            "events": events,
            "top_event": event_label,
            "event_label": event_label,
            "category": category,
            "threat_level": threat_level,
            "confidence": confidence,
            "speech_transcript": transcript.get("text", ""),
